 ...
 * @param {Object} [app]
 * @param {string} [app.name] - the name of the application tracking the event
 * @param {string} [app.version] - the version of the application
 * @param {string} [userAgent] - the useragent string of the user
 * @param {string} [locale]    - the locale string for the user
 * @param {Object} [page]      - an object representing details of the current page
 * @param {string} [page.path] - the path of the current page (e.g /home)
 * @param {string} [page.title] - the title of the current page (e.g 'home')
 * @param {string} [page.url]  - the fully qualified url of the current page
 * @param {Object} [referrer] - for metamask, this is the dapp that triggered an interaction
 * @param {string} [referrer.url] - the origin of the dapp issuing the notification
 */

/**
 * @typedef {Object} MetaMetricsRequiredState
 * @param {bool} participateInMetaMetrics - has the user opted into metametrics
 * @param {string} [metaMetricsId] - the user's metaMetricsId, if they have opted in
 * @param {MetaMetricsEventContext} context - context about the event
 * @param {string} chainId - the chain id of the current network
 * @param {string} network - the name of the current network
 * @param {string} [metaMetricsSendCount] - number of transactions sent, used to add metametricsId
 ...
 * @param {string}  category - category to associate event to
 * @param {boolean} [isOptIn] - happened during opt in/out workflow
 * @param {object}  [properties] - object of custom values to track, snake_case
 * @param {number}  [revenue] - amount of currency that event creates in revenue for MetaMask
 * @param {string}  [currency] - ISO 4127 format currency for events with revenue, defaults to US dollars
 * @param {number}  [value] - Abstract "value" that this event has for MetaMask.
 * @param {boolean} [excludeMetaMetricsId] - whether to exclude the user's metametrics id for anonymity
 * @param {MetaMetricsEventContext} [additionalContext] - additional context to attach to event
 */
 ...
 * @return {(MetaMetricsEventPayload) => undefined} - function to track an event
 */
export function getTrackMetaMetricsEvent (
   ...
   *
   * @param {MetaMetricsEventPayload} - payload for tracking an event
   */
